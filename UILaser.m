...
% private properties
    ...
    % ----------------------------
    function preview_normalize_adjust_255(app, ~)

        % Idee to Combine the three Fuctions in one later but not now
        % because right now we don't have Time because Wafiq Habib

        app.CData = normalize_adjust_255(app,event.Data);
