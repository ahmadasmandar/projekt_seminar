    ...
    methods
        ...
        function success = connect(obj)
            ...
            try
                % switch obj.type
                % case 'gige'
                %     obj.handle=gigecam(obj.type, obj.id, obj.format);
                % otherwise
        ...
        function [success, varargout] = config(obj, config_function)
            ...
            try
                ...
                elseif iscell(config_function) && isa(config_function{1}, 'function_handle')
                    disp("config function else if ")
                    disp(config_function)
        ...
        function success = preview(obj, adjust_function, axes)
            success = false;
            if isa (adjust_function, "matlab.apps.AppBase");
                disp("is a app")
                % obj.received_function=adjust_function.preview_normalize_adjust_255
            else
                disp("is not a app")
            end
            ...
            try
                ...
                if nargin > 1 && isa(adjust_function, 'matlab.apps.AppBase')

                    switch adjust_function
                    case 'UILaser'
                        
                        setappdata(hImage, 'UpdatePreviewWindowFcn', @adjust_function.preview_normalize_adjust_255);
                    case 'UIInfraRed'
                        
                        setappdata(hImage, 'UpdatePreviewWindowFcn', @adjust_function.preview_infrared_adjust);
                    end
                end
